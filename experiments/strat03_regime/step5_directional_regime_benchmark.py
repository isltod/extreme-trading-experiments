...
def run_directional_regime_strategy():
    print(f"[{datetime.now().strftime('%H:%M:%S')}] 4.66년 비트코인 4H 순방향(Directional) 국면 예측 및 매매 전략 시작...")
    ...
    # 1. CatBoost 다중 분류 모델
    print(f"[{datetime.now().strftime('%H:%M:%S')}] 1. CatBoost 순방향 3-Class 모델 훈련 중...")
    ...
    # 2. Random Forest 모델
    print(f"[{datetime.now().strftime('%H:%M:%S')}] 2. Random Forest 순방향 3-Class 모델 훈련 중...")
    ...
    print("\n" + "="*85)
    print("🎯 [CatBoost + RF 앙상블 순방향(Directional) 3-Class Out-of-Sample 미래 분류 성과표]")
    print("="*85)
    print(classification_report(y_test, y_pred_oos, target_names=['0: 횡보(Range)', '1: 상승추세(Bull)', '2: 하락추세(Bear)']))
    ...
    print("="*95)
    print(f"{'전략 세부 모드':<38} | {'총수익률(%)':<12} | {'CAGR(연복리)':<14} | {'최대낙폭(MDD)':<14} | {'샤프지수':<10} | {'승률(%)':<10} | {'총거래수'}")
    print("-"*95)
    ...
    chart_path = os.path.join(chart_dir, "strat03_directional_regime_swing_benchmark.png")
    ...
    for res in sim_results:
        ...
        eq = res['equity_curve']
        name = res['name']
        plt.plot(ts, eq, linewidth=2.0, label=f"{name} (+{res['total_return']:.1f}%, MDD {res['mdd']:.1f}%)")
            
    plt.title("🚀 [STRAT-03] 4H Directional Regime Swing Strategy Benchmark (4.66 Years)", fontsize=14, fontweight='bold')
