...
def run_dual_engine_master_benchmark():
    print(
        f"[{datetime.now().strftime('%H:%M:%S')}] 🚀 [Step 5] 4H 국면 적응형 듀얼 엔진 마스터 파이프라인 시작..."
    )
    ...
    print(
        f"[{datetime.now().strftime('%H:%M:%S')}] 4H 앙상블 관제탑 (CatBoost + RF) 학습 중..."
    )
    ...
    print(
        f"[{datetime.now().strftime('%H:%M:%S')}] 15분봉 시세 데이터와 4H 관제탑 신호 정밀 동기화 중..."
    )
    df_15m_sorted = df_15m.sort_values("timestamp").reset_index(drop=True)
    df_4h_sync = (
        valid_df[["available_time", "p_range", "p_bull", "p_bear"]]
    ...
    merged_15m.loc[
        (merged_15m["p_bull"] >= 0.38)
        & (ema12 > ema26)
        & (ema12.shift(1) <= ema26.shift(1)),
        "trend_signal",
    ] = 1
    # 4H Bear 국면에서 15M EMA 데드크로스 발생 시 숏
    merged_15m.loc[
        (merged_15m["p_bear"] >= 0.38)
        & (ema12 < ema26)
        & (ema12.shift(1) >= ema26.shift(1)),
        "trend_signal",
    ] = -1
    ...
    # -------------------------------------------------------------
    print(
        f"[{datetime.now().strftime('%H:%M:%S')}] 4대 시나리오 4.66년 실거래 시뮬레이션 가동 중..."
    )
    ...
    scenarios = [
        ...
        (
            "시나리오 3: [마스터 듀얼 엔진] 50x 횡보 스캘핑 + 10x 추세 돌파",
            "dual_engine_aggressive",
        ),
        (
            "시나리오 4: [안정형 듀얼 엔진] 25x 횡보 스캘핑 + 5x 추세 돌파",
            "dual_engine_conservative",
        ),
    ]
        ...
        for i in range(len(merged_15m) - 1):
            ...
            if "dual_engine" in s_mode:
                if not in_trend:
                    ...
                    if trend_ended:
                        gain_t = pos_size_t * lev_trend * pnl_pct_t - (
                            pos_size_t * lev_trend * FEE_TAKER
                        )
                        capital += gain_t
                        capital = max(0.0, capital)
                        total_trades += 1
    ...
    print("\n" + "=" * 95)
    print(
        "🏆 [Step 5: 4H 관제탑 + 15M 듀얼 엔진 마스터 4.66년 풀사이클 백테스트 리더보드]"
    )
    print("=" * 95)
    print(
    ...
    for res in sim_results:
        print(
            f"{res['name']:<40} | {res['total_return']:>10.1f}% | {res['cagr']:>12.2f}% | {res['mdd']:>12.2f}% | {res['sharpe']:>8.2f} | {res['win_rate']:>8.1f}% | {res['total_trades']:>6}회 (스캘핑 {res['scalp_trades']} / 추세 {res['trend_trades']})"
        )
    print("=" * 95)
    ...
    print(
        f"• 파산 확률 (Probability of Ruin < $200): {ruin_prob:.2f}% (안전성 검증 완료!)"
    )
    ...
    chart_path = os.path.join(
        chart_dir, "strat03_step5_master_dual_engine_benchmark.png"
    )

    fig, (ax1, ax2) = plt.subplots(2, 1, figsize=(14, 11))
    fig.suptitle(
        "⚡ [STRAT-03 Step 5] 4H Regime-Adaptive Dual Engine Master Integration Benchmark",
        fontsize=14,
        fontweight="bold",
    )
    ...
    for res in sim_results:
        ...
        if "마스터 듀얼 엔진" in name:
            ...
            ax1.plot(
                ts, eq, "k:", alpha=0.5, label=f"{name} ({res['total_return']:.1f}%)"
            )

    ax1.set_title(
        "1. 4-Year Full-Cycle Equity Curves by Strategy Architecture ($1,000 Initial)",
        fontsize=12,
    )
