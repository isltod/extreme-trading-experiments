...
from sklearn.metrics import confusion_matrix, classification_report, accuracy_score, balanced_accuracy_score, matthews_corrcoef
...
def run_direct_regime_trading_benchmark():
    print(f"[{datetime.now().strftime('%H:%M:%S')}] 4.66년 비트코인 4H 데이터 로드 및 3-Class 다중 분류 피처셋 구성 중...")
    ...
    print(f"[{datetime.now().strftime('%H:%M:%S')}] 1. 3-Class CatBoost 모델 훈련 중...")
    cb = CatBoostClassifier(iterations=350, depth=5, learning_rate=0.02, loss_function='MultiClass', random_seed=42, verbose=False)
    cb.fit(X_train, y_train)
    prob_cb_all = cb.predict_proba(X_all)
    
    # 2. 3-Class Random Forest 학습
    print(f"[{datetime.now().strftime('%H:%M:%S')}] 2. 3-Class Random Forest 모델 훈련 중...")
    ...
    xgb = XGBClassifier(n_estimators=250, max_depth=4, learning_rate=0.02, objective='multi:softprob', num_class=3, random_state=42, eval_metric='mlogloss')
    ...
    print("="*85)
    print(classification_report(y_test, y_pred_oos, target_names=['0: 횡보(Range)', '1: 상승(Bull)', '2: 하락(Bear)']))
    print(f"• 3-Class OOS 균형 정확도: {balanced_accuracy_score(y_test, y_pred_oos)*100:.2f}%")
    print(f"• 3-Class OOS 매튜스 상관계수 (MCC): {matthews_corrcoef(y_test, y_pred_oos):.4f}")
    print("="*85)
    ...
    print("="*95)
    print(f"{'전략 모드 및 레버리지':<38} | {'총수익률(%)':<12} | {'CAGR(연복리)':<14} | {'최대낙폭(MDD)':<14} | {'샤프지수':<10} | {'승률(%)':<10} | {'총거래수'}")
    print("-"*95)
    ...
    for lev in [1.0, 3.0, 5.0]:
        ...
        print(f"{label:<38} | {total_ret:>10.1f}% | {cagr:>12.2f}% | {mdd:>12.2f}% | {sharpe:>8.2f} | {win_rate:>8.1f}% | {trades_cnt:>6}회")
    ...
    for res in sim_results:
        ...
        if "1:3" in name and "10x" in name:
            plt.plot(ts, eq, 'r-', linewidth=2.5, label=f"{name} (Best Performance: +{res['total_return']:.1f}%)")
        elif "1:3" in name and "5x" in name:
            plt.plot(ts, eq, 'g-', linewidth=2.0, label=f"{name} (+{res['total_return']:.1f}%)")
        elif "1:3" in name and "3x" in name:
            plt.plot(ts, eq, 'b-', linewidth=1.5, label=f"{name} (+{res['total_return']:.1f}%)")
        elif "연속추종" in name and "3x" in name:
            plt.plot(ts, eq, 'm--', alpha=0.7, label=f"{name} (+{res['total_return']:.1f}%)")
        else:
            plt.plot(ts, eq, alpha=0.5, label=f"{name}")
            
    plt.title("🚀 [STRAT-03] 4H Direct Regime Trading Strategy Benchmark (4.66 Years Full-Cycle)", fontsize=14, fontweight='bold')
